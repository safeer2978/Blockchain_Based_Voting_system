...
function getCandidateNames() public restricted view returns (string[] memory){
    string[] memory names;
...
function getCandidatePartys() public restricted view returns (string[] memory){
    string[] memory partys;
    for(uint i=0;i<candidates.length;i++)
        partys[i] = candidates[i].party;
    return partys;
}
